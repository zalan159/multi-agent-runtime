...
use multi_agent_protocol::{
    create_coding_studio_template, create_claude_workspace_profile, RoleTaskRequest,
...
async fn claude_live_coding_template_writes_file_and_reuses_session() {
    let cwd = create_repo_local_temp_dir("multi-agent-runtime-claude-live");
    ...
    let instance = WorkspaceInstanceParams {
        id: format!("claude-live-{}", unique_suffix()),
        name: "Claude Live Coding".to_string(),
        cwd: Some(cwd.to_string_lossy().to_string()),
    };

    let mut workspace = ClaudeWorkspace::from_template(
        &template,
        &instance,
        &profile,
        ClaudeWorkspaceOptions {
            ...
            turn_timeout: std::time::Duration::from_secs(180),
            ..Default::default()
        },
    );
    ...
    let first = workspace
        ...
        .await
        .expect("first claude task should succeed");
    ...
    let second = workspace
        ...
        .await
        .expect("second claude task should succeed");
    ...
    let second_result = second.dispatch.result_text.unwrap_or_default();
    assert!(second_result.contains("10-prd/group-mentions.md"));
}
